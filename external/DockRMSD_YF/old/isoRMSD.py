...
"""
  ...
  if not maps:
    ...
    if not matches:
      ...
      #                                                  probe.GetProp('_Name')))
      warnings.warn('mol %s does not match mol %s'%(ref.GetProp('_Name'),
                                                    probe.GetProp('_Name')))
    if len(matches) > 1e6: 
  ...
  bestRMS = np.inf #1000.0
  bestRMSD = np.inf #1000.0
  for amap in maps:
    rms=AlignMol(probe,ref,probeConfId,refConfId,atomMap=amap)
    rmsd,rmsd_list = RMSD(probe,ref,amap)
    if rmsd<bestRMSD:
      bestRMSD = rmsd
      rmsd_list_=rmsd_list
    if rms<bestRMS:
      bestRMS=rms
      bestMap = amap
      rmsd_list_=amap
  ...
  return bestRMS, bestRMSD,rmsd_list_
...
def RMSD(probe,ref,amap):
  ...
  for (pi,ri) in amap:
    posp = probe.pos[pi]
    posf = ref.pos[ri]
    rmsd_list.append(dist_2(posp,posf))
    rmsd += dist_2(posp,posf)
  rmsd = math.sqrt(rmsd/atomNum)
  return rmsd,rmsd_list
...
if  __name__ == "__main__":
  ...
  rms,rmsd,rmsd_list_ = GetBestRMSD(probe,ref)
 
  print("\nBest_RMSD: %.3f\nBest_Not_Fit_RMSD: %.3f\n"%(rms,rmsd))
  out = open(sys.argv[3],"w")
  out.write("Best_RMSD: %.3f\nBest_Not_Fit_RMSD: %.3f\n"%(rms,rmsd))
  out.write("RMSD_list: %.3f\n" % (rmsd))
  out.close()
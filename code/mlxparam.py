...
Last update: 2026-02-27 by Junlin_409
version: 1.3.0 更改index0位置
version: 1.2.0 更改loss
"""
...
class MlXParamTrainer:
    ...
    def outer_train(self, sample_path: str, input_files: dict, box: dict,
                    print_mode: bool = False) -> tuple[nn.ParameterDict, list[float], int, list[float]]:
        """
        EX2.数据存储原则(默认):\n
        ...
        index0 = rmsd_eval(input_files, output_file)
        best_index = index0
        if len(best_index) == 0:
        ...
        if speed_up:
            if os.path.exists(f"{sample_path}/mlxparam/new_loss_fix"):
                shutil.rmtree(f"{sample_path}/mlxparam/new_loss_fix")
            os.makedirs(f"{sample_path}/mlxparam/new_loss_fix", exist_ok=True)
            shutil.move(f"/dev/shm/aptheta/mlxparam/{os.path.split(sample_path)[1]}", f"{sample_path}/mlxparam/new_loss_fix")
        return best_weights, best_index, best_epoch, index0

    def record(self, sample_path: str, input_files: dict, box: dict, filepath: str, print_mode: bool = False):
        best_weights, best_index, best_epoch, index_0 = self.outer_train(sample_path, input_files, box, print_mode)
        ws = [
            "Receptor",
